...
# --- PARAMETERS FOR V404 CYGNI ---
mass_sun_equivalent = 9.0
C = const.c 
G = const.G 
M_SUN = 1.98847e30 

# --- HIGH-RES AVALANCHE SEARCH SETTINGS ---
flare_gps = 1243771218 
start_on = flare_gps + (20 * 60)
end_on = flare_gps + (40 * 60)

# --- REFERENCE BACKGROUND (CALIBRATION FROM A QUIET PERIOD) ---
quiet_gps = 1262692818  # Identified baseline for background noise
start_off = quiet_gps
end_off = start_off + (20 * 60)

# Target frequency window for TVERC detection
f0_min, f0_max = 1600, 1900
...
def run_avalanche_search():
    ...
    if bg_spectrum is not None and flare_spectrum is not None:
        ...
        plt.title(f'TVERC AVALANCHE SEARCH: Single Calibration\n'
                  f'Object: V404_Cygni, Window: +{on_offset_min} to +{on_offset_max} min', 
